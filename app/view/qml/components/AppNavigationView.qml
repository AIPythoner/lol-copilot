            ...
            FluIconButton{
                ...
                onClicked: {
                    ...
                    if(pageMode === FluNavigationViewType.Stack){
                        ...
                        // [PATCH] Qt StackView.pop() never destroys popped items. Without this,
                        // every back-press leaves an orphan page that continues to react to
                        // signals (e.g. Lcu.matchDetailChanged rebuilds 10×15 Image loaders
                        // per orphan). Multi-GB leaks on long sessions otherwise.
    ...
    function push(url,argument={}){
        ...
        function stackPush(){
            ...
            var page = nav_stack.find(function(item) {
                return item.url === _urlStr;
            })
            if(page){
                switch(page.launchMode)
                {
                case FluPageType.SingleTask:
                    // [PATCH] Destroy each page popped above the target — same rationale
                    // as the back-button patch. Re-visiting a SingleTask page without
                    // destruction would orphan every page that was above it in the stack.
                ...
                case FluPageType.SingleTop:
                    if (nav_stack.currentItem.url === _urlStr){
                        return
                    }
                    break
            ...
            var options = Object.assign(argument,{url:_urlStr})
            if(pageIndex!==-1){
                nav_stack2.currentIndex = pageIndex
